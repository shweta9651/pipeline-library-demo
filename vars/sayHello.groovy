        ...
        stages {
            ...
            stage ('Build archive') {
		        ...
		        sh "/Applications/${pipelineParams.xcodeVersion}.app/Contents/Developer/usr/bin/xcrun xcodebuild -scheme ${pipelineParams.schemeName} -workspace ${pipelineParams.schemeName}.xcworkspace -configuration ${pipelineParams.config} clean"
	     	        sh "(/Applications/${pipelineParams.xcodeVersion}.app/Contents/Developer/usr/bin/xcrun xcodebuild -scheme ${pipelineParams.schemeName} -workspace ${pipelineParams.schemeName}.xcworkspace -archivePath $HOME/Documents/Build/${pipelineParams.schemeName}/${pipelineParams.schemeName}.xcarchive -configuration ${pipelineParams.config} CODE_SIGN_IDENTITY=${pipelineParams.codeSigningIdentity} PROVISIONING_PROFILE_SPECIFIER=${pipelineParams.provisioningProfileSpecifier} RUN_SWIFT_LINT=FALSE archive) | tee archive.log"
               }
            }    
        } 
